// Shared recipe-generation logic.
//
// Lives here so the synchronous endpoint and the background function run
// EXACTLY the same code — when this was duplicated the two drifted apart, and a
// fix in one silently missed the other.
...
export async function runGeneration(body, user, { timeoutMs } = {}) {
  if (!hasClaude()) {
    const e = new Error('AI recipe generation needs ANTHROPIC_API_KEY to be configured.')
    e.code = 'NO_API_KEY'
    throw e
  }
    ...
    if (body.revise) {
      ...
      if (!prev) { const e = new Error('Nothing to revise.'); e.code = 'BAD_REQUEST'; throw e }
      if (!command) { const e = new Error('Tell me what to change.'); e.code = 'BAD_REQUEST'; throw e }
      ...
      const revised = await claudeJSON({
        timeoutMs,
        system: REVISE_SYSTEM, maxTokens: 5000, messages, model: FAST_MODEL() })
    ...
    if (body.suggest) {
      ...
      if (!what) { const e = new Error('Tell me what you want to cook.'); e.code = 'BAD_REQUEST'; throw e }
...
The cook originally asked for: "${what}". Stay true to the dish named above.` + constraints(prefs, body.pantryItems)
    ...
    return ({ recipes: shaped, recipe: shaped[0] })
}
